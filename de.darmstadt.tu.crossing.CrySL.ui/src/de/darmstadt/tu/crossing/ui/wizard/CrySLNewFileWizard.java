...
public class CrySLNewFileWizard extends Wizard implements INewWizard {
	/**
	 * mirrored from { @see
	...
	protected CrySLNewFileWizardPrimaryPage mainPage;
	...
	public CrySLNewFileWizard() {
		setNeedsProgressMonitor(true);
		setWindowTitle(de.darmstadt.tu.crossing.ui.wizard.Messages.CrySL_Wizard_Title);
	}
	 ...
	 */
	protected CrySLNewFileWizardPrimaryPage createMainPage(String pageName) {
		CrySLNewFileWizardPrimaryPage newpage = new CrySLNewFileWizardPrimaryPage(pageName, templates, selection,
				labelProvider);
		return newpage;
	}
	...
	protected CrySLTemplateFileInfo getFileInfo() {
		return mainPage.getFileInfo();
	}

	@Override
	public boolean performFinish() {
		final CrySLTemplateFileInfo info = getFileInfo();
	...
	protected void doFinish(final CrySLTemplateFileInfo info, final IProgressMonitor monitor) {
		try {
			CrySLFile fileTemplate = (CrySLFile) info.getFileTemplate();
	...
	public IWizardPage getNextPage(IWizardPage page) {
		if (page instanceof NewFileWizardPrimaryPage && hasMoreThenOneTemplate()) {
			CrySLFile selectedTemplate = (CrySLFile) mainPage.getSelectedTemplate();
