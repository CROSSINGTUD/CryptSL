...
public class StatemachineNewDiagramFileWizard extends Wizard {
	 ...
	 * @generated
	 */
	public StatemachineNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot,
			TransactionalEditingDomain editingDomain) {
		assert domainModelURI != null : "Domain model uri must be specified"; //$NON-NLS-1$
		assert diagramRoot != null : "Doagram root element must be specified"; //$NON-NLS-1$
		assert editingDomain != null : "Editing domain must be specified"; //$NON-NLS-1$

		myFileCreationPage = new WizardNewFileCreationPage(Messages.StatemachineNewDiagramFileWizard_CreationPageName,
				StructuredSelection.EMPTY);
		myFileCreationPage.setTitle(Messages.StatemachineNewDiagramFileWizard_CreationPageTitle);
		myFileCreationPage.setDescription(
				NLS.bind(Messages.StatemachineNewDiagramFileWizard_CreationPageDescription, OrderEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage.setFileName(
				StatemachineDiagramEditorUtil.getUniqueFileName(filePath, fileName, "statemachine_diagram")); //$NON-NLS-1$

		diagramRootElementSelectionPage = new DiagramRootElementSelectionPage(
				Messages.StatemachineNewDiagramFileWizard_RootSelectionPageName);
		diagramRootElementSelectionPage.setTitle(Messages.StatemachineNewDiagramFileWizard_RootSelectionPageTitle);
	...
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
				Messages.StatemachineNewDiagramFileWizard_InitDiagramCommand, affectedFiles) {

			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
					throws ExecutionException {
				int diagramVID = StatemachineVisualIDRegistry
						.getDiagramVisualID(diagramRootElementSelectionPage.getModelElement());
				if (diagramVID != OrderEditPart.VISUAL_ID) {
					return CommandResult
							.newErrorCommandResult(Messages.StatemachineNewDiagramFileWizard_IncorrectRootError);
				}
				Diagram diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(),
						OrderEditPart.MODEL_ID, StatemachineDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
		...
		try {
			...
			StatemachineDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			StatemachineDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
	...
	private static class DiagramRootElementSelectionPage extends ModelElementSelectionPage {
		...
		protected boolean validatePage() {
			...
			CreateDiagramViewOperation cvdp = new CreateDiagramViewOperation(
