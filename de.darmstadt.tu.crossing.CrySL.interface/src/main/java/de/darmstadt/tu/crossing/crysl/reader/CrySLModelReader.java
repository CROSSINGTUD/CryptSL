...
import de.darmstadt.tu.crossing.crysl.interfaces.ICrySLPredicateParameter;
...
import de.darmstadt.tu.crossing.crysl.rules.CrySLCondPredicate;
...
import de.darmstadt.tu.crossing.crysl.rules.CrySLSplitter;
import de.darmstadt.tu.crossing.crysl.rules.CrySLValueConstraint;
import de.darmstadt.tu.crossing.crysl.rules.ParEqualsPredicate;
...
public class CrySLModelReader {
	...
	private CrySLArithmeticConstraint convertLiteralToArithmetic(final Constraint expression) {
		...
		return new CrySLArithmeticConstraint(name, new CrySLObject("0", INT), de.darmstadt.tu.crossing.crysl.rules.CrySLArithmeticConstraint.ArithOp.p);
	}
	private CrySLArithmeticConstraint convertArithExpressionToArithmeticConstraint(final Constraint expression) {
		...
		right = new CrySLArithmeticConstraint(
				...
				new CrySLObject(rightValue, getTypeName(ar.getRightExpression(), rightValue)),
				operator);
		return right;
	}
