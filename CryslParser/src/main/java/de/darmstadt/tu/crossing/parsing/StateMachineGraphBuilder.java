...
public class StateMachineGraphBuilder {
    ...
    private final Collection<CrySLMethod> allMethods = new HashSet<>();
    ...
    private static class SubStateMachine {
        ...
        public SubStateMachine(
                final Collection<StateNode> startNodes, final Collection<StateNode> endNodes) {
     ...
     *
     * <p>It is called recursively For a collection of start nodes, which must already be added to
     ...
     *     <p>Having a look at the Crysl Xtext definition for the Order section, we have -----------
     *     <code>
     ...
     * </p>
     * </code> -----------
     *     <p>Based on this definition, the method will build the StateMachine from the Order
     *     section.
     *     <p>This is done by recursively building a sub-StateMachine and connecting it with given
     *     start Nodes and returned end Nodes according to the specific OrderOperator.
     *     <p>Therefore, consider the following cases
     *     <p>1. Order instanceof Primary: In this case, we create a new Node and add a transition
     ...
     *     <p>2. OrderOperator == SEQUENCE: The left-side should occur before the right-side. We
     ...
     *     <p>3. OrderOperator == ALTERNATIVE: Either the left-side or the right-side should occur.
     ...
     *     <p>4. OrderOperator == ZERO_OR_ONE: The Event can occur or be skipped. We therefore build
     ...
     *     <p>5. OrderOperator == ONE_OR_MORE: The Event can occur once or multiple times. We
     ...
     *     <p>5. OrderOperator == ZERO_OR_MORE: This can be seen as (Order)*?. We therefore proceed
    ...
    private SubStateMachine buildSubSMG(final Order order, final Collection<StateNode> startNodes) {
        ...
        Collection<StateNode> end = new HashSet<>();
        Collection<StateNode> start = new HashSet<>();
        ...
        switch (order.getOp()) {
            ...
            case ALTERNATIVE:
                 ...
                 *				Set<StateNode> endNodesWithOutgoingEdges =
